...
def _check_data_types(keys, data, target_dtype):
    ...
    for key in keys:
        assert data.dtype == target_dtype, '{} has data type {} which '\
            'is different than {}'.format(key, data.dtype, target_dtype)
...
def broadcast_data(keys, data, datatype):
    ...
    # motif: use the data directly instead of data[key]
    ...
    if get_model_parallel_rank() == 0:
        ...
        flatten_data = torch.cat(
            [data.contiguous().view(-1) for key in keys], dim=0).cuda()
    else:
